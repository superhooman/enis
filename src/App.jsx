...
const cities = [
  ...
  {
    code: "http://fmalm.nis.edu.kz/Almaty_Fmsh",
    label: "Алматы ФМН",
    value: 2
  },
  {
    code: "http://hbalm.nis.edu.kz/Almaty_Hbsh",
    label: "Алматы ХБН",
    value: 3
  },
  ...
  {
    code: "http://ukk.nis.edu.kz/Oskemen",
    label: "Усть-Каменогорск ХБН",
    value: 16
  },
  {
    code: "http://fmsh.nis.edu.kz/Shymkent_Fmsh",
    label: "Шымкент ФМН",
    value: 17
  },
  {
    code: "http://hbsh.nis.edu.kz/Shymkent_Hbsh",
    label: "Шымкент ХБН",
    value: 18
  }
];
...
const getWidth = subject => {
  ...
  if (subject.MaxISA !== 0) {
    result =
      ...
      subject.ApproveISA / subject.MaxISA * 30;
  } else {
          ...
          {group.goals.map(goal => (
            <div key={goal.Id} className="goal">
              <div className="info">
                ...
                {goal.Changed ? (
                  <span className="date">{timeConv(goal.Changed)}</span>
                ) : (
                  ""
                )}
                <span className={getStatus(goal.Value)}>
                  {getGoalText(goal.Value, locale)}
                </span>
              </div>
              <div className="goal-content">{goal.Description}</div>
              {goal.comment ? (
                <div className="comment">{goal.Comment}</div>
              ) : (
                ""
              )}
            </div>
...
class App extends Component {
  constructor() {
    ...
    this.state = {
      ...
      city: "",
      fromLogin: false,
      locale: parseInt(localStorage.getItem("locale"), 10) || 0
    };
          ...
          <Route
            ...
            render={() =>
              this.state.logged ? (
                ...
                <Login
                  changeLocale={this.locale.bind(this)}
...
class Dashboard extends Component {
  constructor(props) {
    ...
    this.state = {
      redirect: false,
      city: this.props.city || localStorage.getItem("city"),
      journal: this.props.journal || localStorage.getItem("journal"),
      role: this.props.role || localStorage.getItem("role"),
      child: 0,
      childWill: 0,
      quarter: 0,
      modalIMKO: false,
  ...
  getUrl(klass, studentId) {
    ...
    axios({
      ...
      data:
        ...
        klass +
        "&periodId=1" +
        (studentId ? "&studentId=" + studentId : "&studentId=" + this.state.id)
    }).then(response => {
      axios({
        withCredentials: true,
        method: "post",
    ...
    if (selectedOption) {
      if (this.state.journal === "IMKO") {
        axios({
          ...
          url: proxy + this.state.city + "/ImkoDiary/Subjects",
          data:
            "periodId=1&studentId=" +
            self.state.childrenList[selectedOption.value].Id
        }).then(response => {
          if (response.data.success) {
        ...
        axios({
          ...
          data:
            "klassId=" +
            self.state.childrenList[selectedOption.value].klass +
            "&periodId=1&studentId=" +
            self.state.childrenList[selectedOption.value].Id
        }).then(response => {
          axios({
      ...
      if (this.state.journal === "IMKO") {
        axios({
          ...
          data:
            ...
            (self.state.role === "Parent"
              ? "&studentId=" + self.state.childrenList[child].Id
              : "")
        }).then(response => {
          if (response.data.success) {
            var data = self.state.data;
        ...
        axios({
          ...
          data:
            "klassId=" +
            (self.state.role === "Parent"
              ? self.state.childrenList[child].klass
              : this.state.klass) +
            "&periodId=" +
            (id * 1 + 1) +
            (self.state.role === "Parent"
          ...
          <div ref="dashboard" className="dashboard">
            ...
            <div className={this.state.modalShow ? "modal active" : "modal"}>
              ...
              <div className="modal-content">
                ...
                {this.state.modal ? (
                  this.state.journal === "IMKO" ? (
                    <div className="goals-homework">
                      <div className="goals">
                        <h2 className="title">
                          {lang[this.props.locale].goals}
                        </h2>
                        {this.state.goals.length ? (
                          getGoals(this.state.goals, this.props.locale)
                        ) : (
                          <div className="empty">
                            {lang[this.props.locale].empty}
                          </div>
                        )}
                      </div>
                      <div className="homework">
                        <h2 className="title">
                          {lang[this.props.locale].homework}
                        </h2>
                        {this.state.homework.length ? (
                          ...
                          <div className="empty">{lang[this.props.locale].empty}</div>
                        )}
                      </div>
                    </div>
                  ) : (
                    <div className="goals-homework">
                      {this.state.JKO.map((section, index) => (
                        <div key={index} className="homework">
                          {index === 0 ? (
                            <h2 className="title">{lang[this.props.locale].section}</h2>
                          ) : (
                            <h2 className="title">{lang[this.props.locale].quarter}</h2>
                          )}
                          {section.map(topic => (
                            <div key={topic.Id} className="work">
                              <h3 className="name"> {topic.Name} </h3>
                              <span>
                                {(topic.Score === -1 ? 0 : topic.Score) +
                                  " | " +
                                  topic.MaxScore}{" "}
                              </span>
                            </div>
            ...
            <div className="menu">
              {this.state.role === "Parent" ? (
                <Select
                  ...
                  placeholder={lang[this.props.locale].student}
                  searchable={false}
            ...
            <div className="tabs">
              ...
              >
                1 {lang[this.props.locale].quarter}
              </div>
              <div
                onClick={() => {
              ...
              >
                2 {lang[this.props.locale].quarter}
              </div>
              <div
                onClick={() => {
              ...
              >
                3 {lang[this.props.locale].quarter}
              </div>
              <div
                onClick={() => {
              ...
              >
                4 {lang[this.props.locale].quarter}
              </div>
            </div>
            <div className="subjects">
              {this.state.data ? (
                this.state.journal === "IMKO" ? (
                  <div className="subjects">
                    {this.state.data[this.state.child].data[
                      this.state.quarter
                    ].data.map(subject => (
                      <div
                        key={subject.Id}
                        onClick={() => {
                          this.state.role === "Parent"
                            ? this.getGoalIMKO(
                                subject.Id,
                                this.state.childrenList[this.state.child].Id
                              )
                            : this.getGoalIMKO(subject.Id, null);
                        }}
                        className="subject"
                      >
                        <div
                          className={
                            getWidth(subject) === 100
                              ? "progress full"
                              : "progress"
                          }
                          style={{
                        ...
                        <div className="subject-info">
                          <div className="formative">
                            <div className="name">{lang[this.props.locale].fa}</div>
                            <div className="value">
                              {subject.ApproveCnt + " | " + subject.Cnt}
                            </div>
                          </div>
                          <div className="summative">
                            <div className="name">{lang[this.props.locale].sa}</div>
                            <div className="value">
                              {subject.ApproveISA + " | " + subject.MaxISA}
                            </div>
                          </div>
                          <div className="grade">
                            <div className="name">{lang[this.props.locale].mark}</div>
                            <div className="value">
                  ...
                  <div className="subjects">
                    {this.state.data[this.state.child].data[
                      ...
                      >
                        <div
                          className={
                            subject.Score === 100 ? "progress full" : "progress"
                          }
                          style={{
                            width: subject.Score + "%"
                          }}
                        ...
                        <div className="subject-info">
                          <div className="percent">
                            <div className="name">{lang[this.props.locale].percent}</div>
                            <div className="value">
                              {subject.Score ? subject.Score + " | 100" : "N/A"}
                            </div>
                          </div>
                          <div className="grade">
                            <div className="name">{lang[this.props.locale].mark}</div>
                            <div className="value">
                              {subject.Mark ? subject.Mark : "N/A"}
                            </div>
                          </div>
                        </div>
                      </div>
...
class Login extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      journal: localStorage.getItem("journal") || "JKO",
      pin: "",
      password: "",
      remember: false,
      locale: 0
  ...
  componentWillMount() {
    ...
    if (localStorage.getItem("city")) {
      axios({
        withCredentials: true,
        method: "post",
        url: proxy + localStorage.getItem("city") + "/EventCalendar/GetEvents"
      }).then(response => {
        if (response.data.success) {
          self.props.onDone(
    ...
    if (localStorage.getItem("pin")) {
      this.login(localStorage.getItem("pin"), localStorage.getItem("password"), true);
    }
  }
  handleChange = selectedOption => {
    selectedOption
      ...
      .then(response => {
        if (response.data.success) {
          ...
          if (remember) {
            ...
            localStorage.setItem("city", response.data.url);
            localStorage.setItem("journal", self.state.journal);
            localStorage.setItem("cityID", self.state.city);
          }else{
            localStorage.setItem("city", response.data.url);
            localStorage.setItem("journal", self.state.journal);
            localStorage.setItem("cityID", self.state.city);
  ...
  loadRoles(pin, password, remember) {
    ...
    axios({
      ...
      .then(response => {
        if (response.data.success) {
          self.loginWithRole(
            response.data.listRole[0].value,
            pin,
            password,
            remember
          );
      ...
      <div className="login">
        ...
        <div className="content">
          <div className="logo">
            ...
            <h1>eNIS</h1>
            <p>— {lang[this.props.locale].slogan}</p>
          </div>
          <div className="login-cont">
            ...
            >
              <Select
                ...
                placeholder={lang[this.props.locale].city}
                onChange={this.handleChange}
                options={cities}
              />
              <input
            ...
            <div className="group">
              <Checkbox
                ...
                label={lang[this.props.locale].remember}
              />
              <Toggle
                initState={
                  this.state.journal === "IMKO" ? true : false
                }
                onChange={value => {
                  this.setState({
                ...
                label={lang[this.props.locale].imko}
                label0={lang[this.props.locale].jko}
              />
            </div>
            <button
              onClick={() => {
              ...
              className="button"
            >
              {lang[this.props.locale].login}
            </button>
          </div>
          <footer>
            <span
              onClick={() => {
                if (this.props.locale <= 1) {
                  this.props.changeLocale(this.props.locale * 1 + 1);
                } else {
                  this.props.changeLocale(0);
                }
              }}
              className="locale"
            >
